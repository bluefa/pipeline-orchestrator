...
| 16 | stale token **or** expired lease — no-op guarded write-back | `StepReporter.report` after `findByIdForUpdate`: the ownership guard (`ownsLiveClaim`) requires **matching fencing token AND a still-live lease (`claimed_until > now`)**. Fails when: (a) `claimed_by` no longer matches (another worker reclaimed after expiry, or Case A cancel cleared the claim); or (b) the token still matches but the lease has already expired (the pipeline was not yet reclaimed by another worker). In case (b) an expired-lease straggler no-ops its tx2 even if its token still matches, so a concurrent Case-A cancel is never lost — a faithful strengthening of the ADR's fencing premise (ADR-021 Decision 5: `leaseDuration > apiCallTimeout` guarantees a normal tx2 always arrives within the live lease). | `StepReporter.ownsLiveClaim` | the entire report is silently no-op'd — no task or pipeline row is updated; the stale straggler cannot clobber state (ADR-021 Decision 4/5) |
...
- `StepReporter.ownsLiveClaim` is the sole tx2 ownership guard: it requires **matching fencing token AND a still-live lease (`claimed_until > now`)**. An expired-lease straggler no-ops its tx2 even if its token still matches (the pipeline was not yet reclaimed), so a concurrent Case-A cancel is never lost — a faithful strengthening of the ADR's fencing premise.
