...
### 자매 repo 교차검증 후 문서 내부 일관성 3건 (pii-agent-demo#533 대조)

`pii-agent-demo#533`(이 PR을 자매 repo 사본으로 포팅한 PR, ADR 사본은 두 repo가 byte-identical)과
head 파일을 직접 diff한 결과, codex 4라운드가 못 잡은 **문서 내부 일관성** 3건이 드러나 반영했다
(codex는 "ADR↔코드" 정합성만 봤고 "ADR 내부 문구 일관성"은 스코프 밖이었다):

- **"five core enums" 잔재**: §2와 Enums 표는 "four core enums"로 고쳤는데 Consequences 불릿만
  "five"로 남아 있었다 → "four"로 통일(#533과 동일 문구).
- **`check()` 시그니처 혼재**: invariant #1만 코드 시그니처 `check(target, task, attempt)`이고 다른
  5곳은 `check(attempt, task)`였다 → 실제 코드(`TaskType.check(String target, Task task,
  TaskAttempt attempt)`)에 맞춰 6곳 전부 `check(target, task, attempt)`로 통일. (#533은 이 드리프트를
  follow-up으로 지적만 하고 자기 사본은 안 고쳤으므로, 이 부분은 #28이 #533보다 정확해졌다.)
- **`application.yaml` → `application.yml`**: 실제 파일명은 `.yml`(`src/main/resources/application.yml`)
  → 철자 통일. (#533도 미수정 상태였다.)

**#533에서 복사하지 않은 2곳**(자매 repo 전용 현지화라 pipeline-orchestrator엔 오히려 틀림):
설계문서 `terraform-client-and-postcheck-design.md` 링크는 이 repo에 실제 존재하므로 markdown 링크를
유지했고(#533은 그 파일이 없어 de-link), revision-history의 "imported ... from its source repo"는
pipeline-orchestrator 관점에서 정확하므로(#27이 #532에서 문구를 가져옴) 그대로 두었다.
