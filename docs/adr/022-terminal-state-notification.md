...
제외**(`notify_attempts < maxAttempts`)해 정의하고(age 지표 자체는 V1 미배선 — §4 재도입
후속) give-up 행은 **별도 카운트**(§4)로 감시한다 — give-up이 pending age를 무한히
오염시키지 않게 한다. 별도 dead-letter 테이블은
...
in-flight 1건은 lease/fencing이 흡수하는 일반 크래시 경로와 동일).
...
- **알림 전용 지표(두 age를 구분한다 — V1은 정의만, 배선은 재도입 후속)**: 두 age의 종단
  ...
  - **V1이 실제로 배선하는 것**: give-up 수(`countGivenUp`) — NotifyScheduler가 주기 폴링해
...
- **give-up 경보는 필수(MUST).** give-up 행은 자동 재시도가 멈추고 pending-age 지표에서도
  ...
  - **신호(정규 소스 = DB 파생)**: 경보의 **정규 소스는 durable DB 술어**
    ...
    가능하므로 정규 소스로 삼지 않는다. **V1 배선**: NotifyScheduler가 `countGivenUp`을 주기
  ...
  - **복구 절차**: sink를 고친 뒤 운영자가 해당 행의 `notify_attempts`를 리셋하고
...
| 대안 | 판정 | 이유 |
|---|---|---|
| **A. 상태 파생 + `notified_at`(claim/lease 메커니즘 재사용)** | **채택** | 이벤트가 이미 도메인 행에 있어 dual-write 없음; ADR-021 claim/lease/two-tx를 그대로 재사용해 relay-lease 문제 없음; **알림 전달 상태 테이블·relay·pruner 0, 새 테이블 0**(채널 설정은 env — §2, 스키마 절). |
