...
설정은 env 5키(`enabled`/`slack-webhook-url`/`enabled-after`/`environment`/`detail-url-base`)뿐이다.
...
`pipeline.notify.slack-webhook-url`/`pipeline.notify.enabled`; `enabled=true`면 webhook·
`enabled-after`(§5)·`environment`·`detail-url-base`(뒤 둘은 2026-07-10 추가, yml 기본값 있음)가
전부 필수이고 빈 값이면 시작 실패 — fail-fast). gate는 **부팅 시 `enabled`
...
- **소비자 계약**: (a) 멱등 dedupe 키(`pipeline_id`, 파이프라인당 종단 1개) 보관 —
  **최소 보관 기간 = max(파이프라인 행 보존기간, 최대 재시도 horizon + 수동 replay 창)**
  ...
  `environment`/`tenant`를 넣어 dedupe 범위를 한정한다(2026-07-10부터 `environment`는 payload에
  실제로 실린다 — 설정 `pipeline.notify.environment`).
  ...
  `cloud_provider`(CSP enum 이름, 미해석/미지정 시 null), `environment`(설정에서 오는 배포 환경
  태그 — stg/prd 공유 채널 구분), 실패 시 `failed_task`/`error_code`, `detail_url`(아래 링크
  규칙), `schema_version` — 총 10필드(2026-07-10 오너 요청으로 3필드 추가, schema_version 1→2).
  ...
  **링크 규칙(2026-07-10 개정)**: 허용되는 링크는 `detail_url` 하나뿐이다 — 권한 있는 콘솔의
  파이프라인 상세 화면 주소로, **설정된 base(`pipeline.notify.detail-url-base`) + `pipeline_id`만으로
  조립**한다(전용 조립 지점 `toDetailUrl` — id 외의 값은 싣지 않는다). 콘솔이 자체 인증을 하므로
  링크 노출 = id 노출이며 대상 상세는 드러나지 않는다. **대상 정보가 담긴 그 외 링크는 여전히
  금지(MUST NOT)**다 — 이전 판의 "url 필드 전면 금지"를 오너 결정으로 완화한 것이고,
  범용 url 필드를 만들지 않는다는 성질은 유지된다(NotifyPayloadPiiTest가 강제).
  이 규칙은 구현 문서 `NotifyPayload`/Slack 템플릿에도
...
- **채널 설정 테이블 없음** — Slack 채널 설정은 테이블이 아니라 환경변수다(2026-07-09 오너
  ...
  `pipeline.notify.environment`(배포 환경 태그)·`pipeline.notify.detail-url-base`(콘솔 상세 링크
  base — 2026-07-10 추가).
...
| 신호 | 파생 조건 | payload 요지 |
|---|---|---|
| 파이프라인 완료 | `status = DONE` & `notified_at IS NULL` | pipeline id, type, `target_ref`(opaque), `cloud_provider`, `environment`, `detail_url` |
| 파이프라인 실패 | `status = FAILED` & 〃 | 위 + 실패 task와 `error_code` |
| 파이프라인 취소 | `status = CANCELLED` & 〃 | 완료와 동일 |
...
- 2026-07-10: **오너 결정 — 전달 동시성을 단일 트랜잭션(잠금-내-전송)으로 단순화.**
  ...
  동작 값(재시도·주기·타임아웃)은 설정에서 코드 상수로 이동 — 운영 설정은 env 3키만(같은 날
  2차 개정으로 5키가 됨, 아래 항목). give-up
  ...
  사본에는 추후 동일 개정 필요.)
- 2026-07-10 (2차): **오너 요청 — 알림 내용 확장.** payload 허용 필드 7 → **10**
  (`cloud_provider`·`environment`·`detail_url` 추가, `schema_version` 1 → 2). Slack 메시지
  머리글에 배포 환경 태그(`[stg]`/`[prd]`)와 "상세 보기" 링크가 붙고, 필드에 `cloud_provider`가
  추가되며 `target_ref`의 표시 라벨은 운영자 용어인 `target_source`로 바꿨다(payload 필드명은
  `target_ref` 유지). §4 링크 규칙 개정: "url 필드 전면 금지" → **`detail_url`(콘솔 상세 화면,
  설정 base + pipeline_id만으로 조립) 하나만 허용** — 대상 정보가 담긴 그 외 링크 금지는 유지.
  설정 env 3 → 5키: `pipeline.notify.environment`(기본 local)·`pipeline.notify.detail-url-base`
  (기본 로컬 콘솔 주소 — stg/prd는 env로 실제 주소 필수 제공). §4의 멀티환경 dedupe 확장
  (`environment`를 payload에)이 이번에 실현됐다.
