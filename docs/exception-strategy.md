...
| Where handled | caught once in `TaskMachine.runExternalCall` (wrapping execute/check), translated to an `ErrorCode` | written directly to the row by `TaskMachine` |
...
The boundary (`TaskMachine.runExternalCall`, the one helper wrapping execute/check) catches
...
`TaskMachine` wraps **every** `TaskType` call — `execute` and `check` — in one
