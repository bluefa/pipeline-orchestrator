...
| Where handled | caught once in `TaskStateMachine.runExternalCall` (wrapping execute/check/postCheck), translated to an `ErrorCode` | written directly to the row by `TaskStateMachine` |
...
The boundary (`TaskStateMachine.runExternalCall`, the one helper wrapping execute/check/postCheck) catches
...
`TaskStateMachine` wraps **every** `TaskType` call — `execute`, `check`, and `postCheck` — in one
...
- **Observation is separate.** What happened on each attempt (which job, how many polls, the last
  ...
  correctness-only guarantee. `TaskAuditWriter` is resilient to the common case (a missing attempt row is
  a no-op). If
