...
| G4 | Cancel is a non-terminal-guarded transition — it can never resurrect a pipeline a converge already terminalized | Case A `PipelineRepository.cancelIfIdle` (guard `status IN ('RUNNING','PENDING') AND (claimed_by IS NULL OR claimed_until<now)`, 0-row = no-op; PENDING = start-delay wait, LIN-30); Case B `requestCancel` flag (guard `status='RUNNING'`) observed under the claim holder's `FOR UPDATE`; `PipelineControlTest.cancelDoesNotResurrectAPipelineThatAlreadyConvergedToTerminal` + `PipelineExecutionTest.aStaleStragglerCannotResurrectAfterCaseACancel` | ✅ |
