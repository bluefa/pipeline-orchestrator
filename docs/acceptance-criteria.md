...
| # | Criterion | Verified by | Status |
|---|---|---|---|
| C1 | `task_attempt` = one row per attempt, `attempt_no = failCount + 1` | `TaskAuditWriter` + `ObservationTest.aRetryingTaskRecordsOneAttemptRowPerAttemptWithIncreasingAttemptNo` | ✅ |
...
| C3 | The engine never **reads** the observation tables for correctness | `TaskAuditWriter` only writes; `PipelineEngine`/`TaskStateMachine` never query them | ✅ |
...
| # | Criterion | Verified by | Status |
|---|---|---|---|
| E1 | Dispatch is idempotent / at-least-once safe; a crash before storing `job_id` heals by re-dispatch | `TaskStateMachine.dispatch` (re-dispatch on READY); A2 crash-resume test | ✅ |
...
| I2 | External-call failures are exceptions caught at exactly one boundary and translated to `ErrorCode` | the single `TaskStateMachine.runExternalCall` helper (wrapping execute/check/postCheck) + `PipelineEngineTest.aThrowingDispatchIsTreatedAsCheckErrorAndRetriedThenFailed` (+ the CALL_TIMEOUT and postCheck tests) | ✅ |
...
- **TaskAuditWriter ride the engine's `advance` transaction** (no `REQUIRES_NEW` / separate bean). The
...
`TaskStateMachine.runExternalCall` helper wrapping `execute`/`check`/`postCheck`; it catches only Timeout/Failed
