...
- **`Optional`** for "maybe absent" return values; never for fields or parameters. Never `Optional.get()` without a preceding presence check — use `orElseThrow`, `map`, `ifPresent`. Never degrade an `Optional` with `.orElse(null)` only to re-check `== null` — keep it an `Optional` and consume it, or return `Optional` from the source.
- **`SequencedCollection` (Java 21)** — use `list.getFirst()` / `getLast()`, not `list.get(0)` / `list.get(list.size()-1)`. The intent-named accessor throws `NoSuchElementException` on empty instead of a stray `IndexOutOfBounds`.
...
1. **Intention-revealing names.** Classes = nouns (`PipelineCreationService`); methods = verbs (`resolveRecipe`, `derivePipelineStatus`); booleans read as predicates (`isTerminal`, `occupiesSlot`). No abbreviations except universally-known ones. A name that needs a comment to explain it is the wrong name. **Role vocabulary, not jargon:** name a step/method by its role in the flow — `claim` / `run` / `writeBack` (a method that persists a result back is `writeBack(...)`, not `report(...)`) — never positional jargon like `tx1`/`tx2`/`phaseA`/`step1`. A domain noun carries the domain: `terraformSlot`, not a bare `slot`. Role-based collection field names (`tasks`, `pipelines`, `settings`) are correct — reveal the role, don't echo the type (`taskRepository`). Extract a non-trivial inline boolean/compound expression into a named predicate (`isReadyToDispatch(task)`) instead of leaving a multi-clause `&&`/`||` puzzle in the condition.
...
7. **Meaningful error handling.** Throw specific exceptions (`UnknownHandlerException`), never swallow (`catch (Exception e) {}`), never `catch` and `return null`. Catch only what you can handle; let the rest propagate. The `23505 → return existing` catch is targeted (catch `DataIntegrityViolationException`, verify it is the unique violation, then act). **API boundary:** an exception that can reach a controller is a dedicated `OrchestrationException` subtype (e.g. `PipelineNotFoundException`) carrying a stable HTTP status (400/404/409) and a stable `code` built from the `OrchestrationErrorCode` enum (`PREFIX + name()` — never a magic `"ORCHESTRATION_..."` string re-spelled elsewhere). Wrap a raw infra exception (`DataIntegrityViolationException`, etc.) into that controlled type at the service boundary; `GlobalAdvice` (`@RestControllerAdvice`) then handles `OrchestrationException` in ONE place plus a catch-all that logs the cause. A missing required argument on a public entry point fails as such a typed contract exception, not a bare NPE or generic `IllegalArgumentException`.
