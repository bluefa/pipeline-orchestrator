...
public class Pipeline {
    ...
    /**
     * 이 실행을 요청한 사람. 검증된 계정에서 BFF가 채워 보내며 오케스트레이터는 기록만 한다. 승인 게이트가
     * 있는 레시피에서는 필수다 — 요청자를 모르는 승인 요청은 감사가 성립하지 않는다.
     *
     * 실행 기록만으로는 무엇이 돌았는지까지만 알 수 있어, 게이트가 없는 실행에서도 있으면 기록한다.
     */
    @Column(name = "requested_by", updatable = false, length = REQUESTED_BY_LENGTH)
    private String requestedBy;

    /**
     * 요청자가 이 실행에 남기는 사유(승인 게이트가 있으면 승인자가 읽는다). 자유 텍스트라 길이를 경계에서 검증하고(초과는 거절),
     * 저장 후에는 바꾸지 않는다.
     */
