 ...
 * awaitApprovalCount는 승인 대기(AWAIT_APPROVAL)를 따로 노출하며 역시 totalCount에 포함된다 — 응답이
 * "상태별 수치의 합 = 전체"라는 구조라, 합계에만 더하고 필드를 빼면 노출된 값과 전체가 어긋난다.
...
public record PipelineStatistics(
        ...
        @JsonProperty("await_approval_count") long awaitApprovalCount,
