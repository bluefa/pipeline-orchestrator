 ...
 * reconciler never reads it, and losing it costs only debuggability, never correctness.
 * {@code attemptNo = task.failCount + 1} at the moment the attempt begins, so a task that retried
 * twice and then failed leaves three rows (1, 2, 3).
 *
...
public class TaskAttempt {
    ...
    /** The attempt outcome: IN_PROGRESS while running, then DONE or FAILED. */
