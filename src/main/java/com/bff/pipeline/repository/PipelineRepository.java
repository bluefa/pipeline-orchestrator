 ...
 * RUNNING·PENDING(시작 지연 대기)·AWAIT_APPROVAL(승인 대기)을 함께 본다 — 뒤의 둘은 미래 시각까지 잠자다
 * 그 시각이 오면 claim되고, 그 트랜잭션에서 RUNNING으로 전이한다(LIN-30 / 승인 게이트 ADR §결정 2).
 * {@code requestCancel}(Case B)은 live-lease 전용이라 RUNNING만 본다(나머지 둘은 미claim이라 항상 Case A로
 * 취소됨).
 */
public interface PipelineRepository extends JpaRepository<Pipeline, Long> {
    ...
    @Query("select p from Pipeline p where p.status in "
            ...
            + "com.bff.pipeline.enums.PipelineStatus.AWAIT_APPROVAL) "
    ...
    /**
     * 알림 대상 행을 잠그는 실제 질의다. 알림 대상이 되려면 다음을 전부 만족해야 한다.
     * - 파이프라인이 끝난 상태다(DONE, FAILED, CANCELLED 중 하나).
     * - 아직 알림을 보낸 적이 없다({@code notified_at}이 비어 있다).
     * - 알림 기능 도입 시각 이후에 끝났다({@code last_activity_at >= enabledAfter}). 끝난 행은 그 뒤로
     *   갱신되지 않으므로 lastActivityAt이 곧 끝난 시각이다. 이 조건 덕에 옛 행에 표시를 남기는 마이그레이션
     *   없이도, 알림을 처음 켜는 순간 과거에 끝난 파이프라인 전부가 쏟아지는 사태를 막는다.
     *   빼면 옛 파이프라인 전부가 다시 알림 대상이 되므로 계속 유지해야 한다.
     ...
     *
     * 잠금 방식은 실행용 {@link #lockClaimableDuePipelines}와 같다(MySQL 8에서 FOR UPDATE SKIP LOCKED).
     * 다른 서버가 전송 중이라 잠가 둔 행은 기다리지 않고 건너뛴다 — 별도의 점유 컬럼 없이 이 행 잠금이
     * "지금 누가 보내는 중인가"를 나타낸다. 전송과 결과 기록이 이 잠금을 쥔 한 트랜잭션 안에서 끝난다.
    ...
    /**
     * 전송 실패가 상한에 닿아 자동 재시도를 멈춘 채 아직 알림이 나가지 못한 파이프라인 수를 센다.
     * 0보다 크면 사람이 개입해야 한다는 신호다({@code notify_attempts}를 0으로 되돌리면 다시 보낸다).
     * TerminalNotifier가 주기적으로 읽어 0보다 크면 ERROR 로그로 경보한다.
     */
