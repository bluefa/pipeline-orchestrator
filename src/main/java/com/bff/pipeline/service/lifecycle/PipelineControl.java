 ...
 *       지운다(가드 {@code status='RUNNING' AND (claimed_by IS NULL OR claimed_until<now)}). 한 행이라도 갱신되면
 *       비종료 task를 모두 CANCELLED로 수렴시킨다. lease가 만료된 straggler는 token이 지워져 tx2 소유권 가드에서 no-op이 된다.</li>
 ...
 * </ul>
 *
 * <p>claim과 Case A가 같은 pipeline 행을 두고 경합하면 먼저 커밋한 쪽이 이긴다. 워커가 claim을 마치고 나면 Case A는 0행이
 * 되어 Case B로 폴백한다. 두 경로 모두 {@code status='RUNNING'} 가드로 종료된 행을 보호하므로(멱등 재취소도 0행),
 * 어느 쪽도 워커 tx2와 live token을 공유하지 않는다. 따라서 terminal resurrection이 불가능하고, 별도 {@code status} 가드가 필요 없다.
 */
@Service
