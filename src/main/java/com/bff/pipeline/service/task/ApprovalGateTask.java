 ...
 *
 * {@code execute}가 하는 일은 승인 만료 시각을 정하는 것뿐이다(지금 + 설정된 대기 상한). 그것을 담아
 * "승인 대기로 들어간다"는 결과를 돌려주면, 엔진이 write-back 트랜잭션에서 승인 요청 행을 만들고 태스크를
 * AWAIT_APPROVAL로 옮긴다. 여기서 행을 직접 쓰지 않는 이유는 이 메서드가 트랜잭션 밖에서 돌기 때문이다 —
 * 요청 행 생성과 상태 전이는 반드시 한 트랜잭션 안에서 함께 커밋돼야 한다.
 *
...
public class ApprovalGateTask implements TaskType {
    ...
    public DispatchResult execute(String target, Task task) {
        return DispatchResult.awaitApproval(clock.instant().plus(approvalSettings.timeout()));
    }
