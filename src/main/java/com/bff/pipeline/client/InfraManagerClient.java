 ...
 *
 * 실패는 세 예외로만 전달한다. 호출 실패의 신호는 {@code exception} 패키지의 닫힌 어휘 셋으로 닫혀 있다
 ...
 *
 * 모든 dispatch는 멱등하다(ADR-016 §5). 중복 submit도 인프라를 올바른 상태로 유지하므로
 * 최소 한 번(at-least-once) 재dispatch가 안전하다.
 */
public interface InfraManagerClient {

    /**
     * operation에 해당하는 Terraform 잡(들)을 디스패치하고 원시 dispatch response를 반환한다(ADR-016 ed97ec0 §5).
