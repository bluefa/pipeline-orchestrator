 ...
 * provider별 flow 확정 후).
 *
 * displayName은 "실행 단위 + 테라폼 + job 타입(Plan/Apply/Destroy)" 표기로 통일한다 — 운영자가 목록에서
 * 실행 단위와 동작을 한눈에 식별하게 한다.
 *
...
public enum TaskDefinition {
    ...
    // ══ AWS Service Level Terraform ══

    AWS_SERVICE_PLAN_V1(CloudProvider.AWS, TaskOperation.AWS_SERVICE_TF_PLAN,
            "AWS Service Level 테라폼 Plan", "대상 AWS service level 인프라의 Terraform plan을 실행해 변경 내용을 산출한다.",
    ...
    AWS_SERVICE_APPLY_V1(CloudProvider.AWS, TaskOperation.AWS_SERVICE_TF_APPLY,
            "AWS Service Level 테라폼 Apply", "대상의 AWS service level 인프라를 Terraform apply로 구성한다.",
    ...
    AWS_SERVICE_DESTROY_V1(CloudProvider.AWS, TaskOperation.AWS_SERVICE_TF_DESTROY,
            "AWS Service Level 테라폼 Destroy", "대상의 AWS service level 인프라를 Terraform destroy로 제거한다.",
    ...
    AWS_BDC_COMMON_PLAN_V1(CloudProvider.AWS, TaskOperation.AWS_BDC_COMMON_TF_PLAN,
            "AWS BDC Common 테라폼 Plan", "AWS BDC service level common 인프라의 Terraform plan을 실행해 변경 내용을 산출한다.",
            TaskExecutionSpec.terraform(TerraformJobType.PLAN,
    ...
    AWS_BDC_COMMON_APPLY_V1(CloudProvider.AWS, TaskOperation.AWS_BDC_COMMON_TF_APPLY,
            "AWS BDC Common 테라폼 Apply", "AWS BDC service level common 인프라를 Terraform apply로 구성한다.",
            TaskExecutionSpec.terraform(TerraformJobType.APPLY,
    ...
    AWS_BDC_COMMON_DESTROY_V1(CloudProvider.AWS, TaskOperation.AWS_BDC_COMMON_TF_DESTROY,
            "AWS BDC Common 테라폼 Destroy", "AWS BDC service level common 인프라를 Terraform destroy로 제거한다.",
            TaskExecutionSpec.terraform(TerraformJobType.DESTROY,
    ...
    AWS_BDC_SERVICE_LEVEL_PLAN_V1(CloudProvider.AWS, TaskOperation.AWS_BDC_SERVICE_LEVEL_TF_PLAN,
            "AWS BDC Service Level 테라폼 Plan", "AWS BDC service level 인프라의 Terraform plan을 실행해 변경 내용을 산출한다.",
            TaskExecutionSpec.terraform(TerraformJobType.PLAN,
    ...
    AWS_BDC_SERVICE_LEVEL_APPLY_V1(CloudProvider.AWS, TaskOperation.AWS_BDC_SERVICE_LEVEL_TF_APPLY,
            "AWS BDC Service Level 테라폼 Apply", "AWS BDC service level 인프라를 Terraform apply로 구성한다.",
            TaskExecutionSpec.terraform(TerraformJobType.APPLY,
    ...
    AWS_BDC_SERVICE_LEVEL_DESTROY_V1(CloudProvider.AWS, TaskOperation.AWS_BDC_SERVICE_LEVEL_TF_DESTROY,
            "AWS BDC Service Level 테라폼 Destroy", "AWS BDC service level 인프라를 Terraform destroy로 제거한다.",
            TaskExecutionSpec.terraform(TerraformJobType.DESTROY,
    ...
    // ══ GCP Service Terraform ══

    GCP_SERVICE_PLAN_V1(CloudProvider.GCP, TaskOperation.GCP_SERVICE_TF_PLAN,
            "GCP Service 테라폼 Plan", "GCP 서비스 인프라의 Terraform plan을 실행해 변경 내용을 산출한다.",
            TaskExecutionSpec.terraform(TerraformJobType.PLAN,
    ...
    GCP_SERVICE_APPLY_V1(CloudProvider.GCP, TaskOperation.GCP_SERVICE_TF_APPLY,
            "GCP Service 테라폼 Apply", "GCP 서비스 인프라를 Terraform apply로 구성한다. BDC apply보다 먼저 수행돼야 한다(서버 강제).",
            TaskExecutionSpec.terraform(TerraformJobType.APPLY,
    ...
    GCP_SERVICE_DESTROY_V1(CloudProvider.GCP, TaskOperation.GCP_SERVICE_TF_DESTROY,
            "GCP Service 테라폼 Destroy", "GCP 서비스 인프라를 Terraform destroy로 제거한다. BDC destroy 이후에 수행돼야 한다(서버 강제).",
            TaskExecutionSpec.terraform(TerraformJobType.DESTROY,
    ...
    GCP_BDC_PLAN_V1(CloudProvider.GCP, TaskOperation.GCP_BDC_TF_PLAN,
            "GCP BDC 테라폼 Plan", "GCP BDC 인프라의 Terraform plan을 실행해 변경 내용을 산출한다.",
            TaskExecutionSpec.terraform(TerraformJobType.PLAN,
    ...
    GCP_BDC_APPLY_V1(CloudProvider.GCP, TaskOperation.GCP_BDC_TF_APPLY,
            "GCP BDC 테라폼 Apply", "GCP BDC 인프라를 Terraform apply로 구성한다. 서비스 apply 이후에 수행돼야 한다(서버 강제).",
            TaskExecutionSpec.terraform(TerraformJobType.APPLY,
    ...
    GCP_BDC_DESTROY_V1(CloudProvider.GCP, TaskOperation.GCP_BDC_TF_DESTROY,
            "GCP BDC 테라폼 Destroy", "GCP BDC 인프라를 Terraform destroy로 제거한다. 서비스 destroy보다 먼저 수행돼야 한다(서버 강제).",
            TaskExecutionSpec.terraform(TerraformJobType.DESTROY,
    ...
    AZURE_BDC_PLAN_V1(CloudProvider.AZURE, TaskOperation.AZURE_BDC_TF_PLAN,
            "Azure BDC 테라폼 Plan", "Azure BDC 인프라의 Terraform plan을 실행해 변경 내용을 산출한다.",
            TaskExecutionSpec.terraform(TerraformJobType.PLAN,
    ...
    AZURE_BDC_APPLY_V1(CloudProvider.AZURE, TaskOperation.AZURE_BDC_TF_APPLY,
            "Azure BDC 테라폼 Apply", "Azure BDC 인프라를 Terraform apply로 구성한다.",
            TaskExecutionSpec.terraform(TerraformJobType.APPLY,
    ...
    AZURE_BDC_DESTROY_V1(CloudProvider.AZURE, TaskOperation.AZURE_BDC_TF_DESTROY,
            "Azure BDC 테라폼 Destroy", "Azure BDC 인프라를 Terraform destroy로 제거한다.",
            TaskExecutionSpec.terraform(TerraformJobType.DESTROY,
    ...
    IDC_CX_PLAN_V1(CloudProvider.IDC, TaskOperation.IDC_CX_TF_PLAN,
            "IDC CX 테라폼 Plan", "IDC CX 인프라의 Terraform plan을 실행해 변경 내용을 산출한다.",
            TaskExecutionSpec.terraform(TerraformJobType.PLAN,
    ...
    IDC_CX_APPLY_V1(CloudProvider.IDC, TaskOperation.IDC_CX_TF_APPLY,
            "IDC CX 테라폼 Apply", "IDC CX 인프라를 Terraform apply로 구성한다. BDP apply보다 먼저 수행돼야 한다(서버 강제).",
            TaskExecutionSpec.terraform(TerraformJobType.APPLY,
    ...
    IDC_CX_DESTROY_V1(CloudProvider.IDC, TaskOperation.IDC_CX_TF_DESTROY,
            "IDC CX 테라폼 Destroy", "IDC CX 인프라를 Terraform destroy로 제거한다.",
            TaskExecutionSpec.terraform(TerraformJobType.DESTROY,
    ...
    IDC_BDP_PLAN_V1(CloudProvider.IDC, TaskOperation.IDC_BDP_TF_PLAN,
            "IDC BDP 테라폼 Plan", "IDC BDP 인프라의 Terraform plan을 실행해 변경 내용을 산출한다.",
            TaskExecutionSpec.terraform(TerraformJobType.PLAN,
    ...
    IDC_BDP_APPLY_V1(CloudProvider.IDC, TaskOperation.IDC_BDP_TF_APPLY,
            "IDC BDP 테라폼 Apply", "IDC BDP 인프라를 Terraform apply로 구성한다. CX apply 이후에 수행돼야 한다(서버 강제).",
            TaskExecutionSpec.terraform(TerraformJobType.APPLY,
    ...
    IDC_BDP_DESTROY_V1(CloudProvider.IDC, TaskOperation.IDC_BDP_TF_DESTROY,
            "IDC BDP 테라폼 Destroy", "IDC BDP 인프라를 Terraform destroy로 제거한다. pod 삭제를 동반한다.",
            TaskExecutionSpec.terraform(TerraformJobType.DESTROY,
