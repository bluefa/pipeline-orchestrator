 ...
 * pipeline_id로 권한 있는 콘솔에서 조회), (b) failed_task는 닫힌 recipe task 키 어휘(taskName =
 * TaskOperation mechanism, 부팅 시 TaskTypeRegistry가 등록을 검증하는 닫힌 집합)에 속한다 — provider/운영자
 * 유래 raw 명 금지, (c) raw 연결 식별자(host/port/credential/DB명) 패턴이 직렬화 결과에 없다 —
 * toTargetRef 밖의 경로로 민감 값이 새면 실패한다, (d) 비-FAILED payload는 failed_task/error_code가 null이다.
 * buildPayload가 sequence 최소 FAILED task에서 값을 채우는 동작은 NotifyLifecycleTest가 검증한다.
 */
class NotifyPayloadPiiTest {
    ...
    @Test
    void failedTaskComesFromTheClosedTaskNameVocabulary() {
        Set<String> closedTaskNames = Arrays.stream(TaskOperation.values())
                .map(TaskOperation::mechanism)
                .collect(Collectors.toSet());

        // 닫힌 어휘 자체가 고정돼 있고(TaskType.NAME들 — 부팅 시 TaskTypeRegistry가 등록을 검증한다)
        assertThat(closedTaskNames).containsExactlyInAnyOrder(
                TaskOperation.Mechanism.TERRAFORM_JOB, TaskOperation.Mechanism.CONDITION_CHECK);
        // 행에 저장되는 taskName(= failed_task의 원천, PipelineInserter가 definition.mechanism()에서 파생)은
        // 전부 이 어휘에서만 나온다 — raw provider/운영자 명이 낄 자리가 없다
        assertThat(Arrays.stream(TaskDefinition.values()).map(TaskDefinition::mechanism))
                .allMatch(closedTaskNames::contains);
        assertThat(failedPayload().failedTask()).isIn(closedTaskNames);
    }
    ...
    private static NotifyPayload failedPayload() {
        return NotifyPayload.builder()
                ...
                .failedTask(TaskOperation.Mechanism.TERRAFORM_JOB)
